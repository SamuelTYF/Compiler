    ...
    Get()
    {
        ...
        while(this.Index<=this.Length)
        {
            ...
            switch(this.State)
            {
				...
				break;
				case 1:
					if(char=="'" | ("0"<=char && char<="9") |("A"<=char && char<="Z") | ("a"<=char && char<="z")){this.Push(char);this.State=5;this.Index++;}
					else {return this.Error(char);}
				break;
				case 2:
					if(char==" " | char==";" | char=='%' | char=='/' | char=='?' | char=="." | char=='=' | ("("<=char && char<="-") | ("0"<=char && char<=":") | ("<"<=char && char<=">") | ("A"<=char && char<="]") | ("a"<=char && char<="}")){this.Push(char);this.State=2;this.Index++;}
